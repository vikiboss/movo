...
import { md5, timestamp, NOOP, lock, OnlineStatus, hide } from './common'
import { BaseClient, Platform, pb, generateShortDevice } from './core'
...
import { parseDmMessageId, parseGroupMessageId, Image } from './message'

import type { Gender } from './common'
import type { ShortDevice, Domain } from './core'
import type { StrangerInfo, FriendInfo, GroupInfo, MemberInfo } from './entities'
import type { EventMap } from './events'
import type { Forwardable, Quotable, Sendable, ImageElem } from './message'
...
export class Client extends BaseClient {
  ...
  async login(password?: string | Buffer) {
    ...
    try {
      ...
      else return this.sig.qrsig.length ? this.qrcodeLogin() : this.fetchQrcode()
    }
  }
  ...
  /** 制作转发消息 */
  makeForwardMsg(fake: Forwardable[], title?: string, desc?: string, dm = false) {
    return (dm ? this.pickFriend : this.pickGroup)(this.uin).makeForwardMsg(fake, title, desc)
  }
  ...
  protected _msgExists(from: number, type: number, seq: number, time: number) {
    if (timestamp() + this.sig.time_diff - time >= 60 || time < this.stat.start_time) return true
