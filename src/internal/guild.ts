...
import { pb } from '../core'
import { parse, Converter } from '../message'

import type { Client } from '../client'
import type { MessageElem, Sendable } from '../message'
...
export class GuildMessageEvent {
  ...
  constructor(proto: pb.Proto) {
    ...
    if (head2[1] !== 3840) throw new Error('unsupport guild message type')
...
export function guildMsgListener(this: Client, payload: Buffer) {
  ...
  if (msg.sender.tiny_id === this.tiny_id && this.config.ignore_self) return
  this.logger.info(
    `recv from: [Guild: ${msg.guild_name}, Member: ${msg.sender.nickname}]` + msg.raw_message
  )
