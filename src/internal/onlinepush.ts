import { NOOP, timestamp, OnlineStatus } from '../common'
...
export function onlinePushListener(this: Client, payload: Buffer, seq: number) {
  ...
  const list = nested[2]
  const v = list[0]
  const rubbish = jce.encodeNested([this.uin, v[1], v[3], v[8], 0, 0, 0, 0, 0, 0, 0])
...
export function onlinePushTransListener(this: Client, payload: Buffer, seq: number) {
  ...
  if (proto[3] === 44) {
    if (buf[5] === 0 || buf[5] === 1) {
      ...
      if (user_id === this.uin) this.gl.get(gid)!.admin_flag = set
    ...
    let operator_id
    let dismiss = false
    const member = this.gml.get(gid)?.get(user_id)
    if (buf[9] === 0x82 || buf[9] === 0x2) {
      ...
      if (buf[9] === 0x01 || buf[9] === 0x81) dismiss = true
      if (user_id === this.uin) {
        this.gl.delete(gid)
        this.gml.delete(gid)
...
export function groupMsgListener(this: Client, payload: Buffer) {
  ...
  if (msg.user_id === this.uin && this.config.ignore_self) return

  // 分片专属屎山
  if (msg.pktnum > 1) {
    const k = [this.uin, msg.group_id, msg.user_id, msg.div].join()
    if (!fragmap.has(k)) fragmap.set(k, [])
    const arr = fragmap.get(k)!
    arr.push(msg)
    setTimeout(() => fragmap.delete(k), 5000)
    if (arr.length !== msg.pktnum) return
  ...
  if (msg.raw_message) {
    ...
    this.logger.info(
      `recv from: [Group: ${msg.group_name}(${msg.group_id}), Member: ${
...
export function discussMsgListener(this: Client, payload: Buffer, seq: number) {
  ...
  if (msg.raw_message) {
    ...
    this.logger.info(
      `recv from: [Discuss: ${msg.discuss_name}(${msg.discuss_id}), Member: ${msg.sender.card}(${msg.sender.user_id})] ` +
        msg
    )
