...
import type { PrivateMessageEvent } from '../events'
...
export async function pbGetMsg(this: Client) {
  if (!this._sync_cookie) this._sync_cookie = buildSyncCookie(this.sig.session.readUInt32BE())
  ...
  const rubbish = []
  if (proto[3]) this._sync_cookie = proto[3].toBuffer()
  if (proto[1] > 0 || !proto[5]) return
  if (!Array.isArray(proto[5])) proto[5] = [proto[5]]
  for (const v of proto[5]) {
    if (!v[4]) continue
    if (!Array.isArray(v[4])) v[4] = [v[4]]
    for (const msg of v[4]) {
      const item = { ...msg[1] }
      item[3] = 187
      rubbish.push(item)
  ...
  if (rubbish.length) this.writeUni('MessageSvc.PbDeleteMsg', pb.encode({ 1: rubbish }))
}
...
async function handleSyncMsg(this: Client, proto: pb.Proto) {
  ...
  const from = head[1]
  const to = head[2]
  if (from === this.uin && from !== to) return
  if (!typelist.includes(type)) return
  if (this._msgExists(from, type, head[5], head[6])) return
  ...
  // 私聊消息
    ...
    if (msg.raw_message) {
      ...
      else if (msg.sub_type === 'self') msg.sender.nickname = this.nickname
...
export function pushReadedListener(this: Client, payload: Buffer) {
  ...
  for (const v of nested[1]) {
    this.em('sync.read.private', {
      user_id: v[0],
      timestamp: v[1]
    })
  }
  for (const v of nested[2]) {
