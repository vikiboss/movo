...
import type { Gender, OnlineStatus } from '../common'
import type { Image } from '../message'
...
export async function setStatus(this: Client, status: OnlineStatus) {
  ...
  if ([Platform.Watch, Platform.aPad].includes(this.config.platform as Platform)) return false
  const d = this.device
  ...
  if (ret) this.status = Number(status)
  return ret
}
  ...
  const body = pb.encode({
    ...
    5: {
      ...
      4: Buffer.concat([
        ...
        buf,
        Buffer.from([
          0x91, 0x04, 0x00, 0x00, 0x00, 0x00, 0x92, 0x04, 0x00, 0x00, 0x00, 0x00, 0xa2, 0x04, 0x00,
          0x00, 0x00, 0x00, 0xa3, 0x04, 0x00, 0x00, 0x00, 0x00
        ])
      ]),
      5: 0
    },
...
export async function delStamp(this: Client, id: string | string[]) {
  ...
  for (const s of id) this.stamp.delete(s)
}
...
export async function addClass(this: Client, name: string) {
  ...
  buf.fill(name, 2)
  const SetGroupReq = jce.encodeStruct([0, this.uin, buf])
...
export async function delClass(this: Client, id: number) {
  const SetGroupReq = jce.encodeStruct([2, this.uin, Buffer.from([Number(id)])])
  const body = jce.encodeWrapper(
    { SetGroupReq },
    'mqq.IMService.FriendListServiceServantObj',
    'SetGroupReq'
...
export async function renameClass(this: Client, id: number, name: string) {
  ...
  buf.writeUInt8(len, 1)
  buf.fill(name, 2)
  const SetGroupReq = jce.encodeStruct([1, this.uin, buf])
  const body = jce.encodeWrapper(
    { SetGroupReq },
    'mqq.IMService.FriendListServiceServantObj',
    'SetGroupReq'
...
export async function loadFL(this: Client) {
  ...
  while (true) {
    ...
    this.classes.clear()
    for (const v of nested[14]) this.classes.set(v[0], v[1])
    for (const v of nested[7]) {
      const uid = v[0]
      const info = {
        user_id: uid,
        nickname: v[14] || '',
  ...
  for (const [uid, _] of this.fl) {
    if (!set.has(uid)) this.fl.delete(uid)
  }
}
...
export async function loadSL(this: Client) {
  ...
  for (const [uid, _] of this.sl) {
    if (!set.has(uid)) this.sl.delete(uid)
  }
}

export async function loadGL(this: Client) {
  const GetTroopListReqV2Simplify = jce.encodeStruct([this.uin, 0, null, [], 1, 8, 0, 1, 1])
  const body = jce.encodeWrapper(
...
export async function loadBL(this: Client) {
  ...
  if (!protos) return
  if (!Array.isArray(protos)) protos = [protos]
  for (const proto of protos) this.blacklist.add(proto[1])
}
...
export class OcrResult {
  ...
  constructor(proto: pb.Proto) {
    ...
    if (!Array.isArray(proto[1])) proto[1] = [proto[1]]
    for (const p of proto[1]) {
      this.wordslist.push({
        words: p[1]?.toString() || '',
  ...
  toString() {
    let str = ''
    for (const elem of this.wordslist) str += elem.words
    return str
  }
}
