...
export abstract class Contactable {
  ...
  async uploadImages(imgs: Image[] | ImageElem[]) {
    ...
    let n = 0
    while (imgs.length > n) {
      let rsp = await (this.dm ? this._offPicUp : this._groupPicUp).call(
        this,
        imgs.slice(n, n + 20) as Image[]
      )
  ...
  protected async _preprocess(content: Sendable, source?: Quotable) {
    try {
      ...
      if (source) converter.quote(source)
      if (converter.imgs.length) await this.uploadImages(converter.imgs)
      return converter
    } catch (e: any) {
  ...
  async uploadVideo(elem: VideoElem): Promise<VideoElem> {
    ...
    if (rsp[1]) throw new Error(String(rsp[2]))
    if (!rsp[7]) {
      const md5 = await md5Stream(createReadable(thumb, file))
      await highwayUpload.call(this.c, createReadable(thumb, file), {
...
<msg brief="[聊天记录]" m_fileName="${uuid().toUpperCase()}" action="viewMultiMsg" tSum="${
  ...
  async getVideoUrl(fid: string, md5: string | Buffer) {
    ...
    if (rsp[1] !== 0) drop(rsp[1], '获取视频下载地址失败')
    const obj = rsp[9]
...
async function* concatStreams(readable1: Readable, readable2: Readable) {
  for await (const chunk of readable1) yield chunk
