...
import { rand2uuid, parseDmMessageId, parseGroupMessageId } from './message'
...
export class Converter {
  ...
  public constructor(content: Sendable, private ext?: ConverterExt) {
    if (typeof content === 'string') {
      ...
      for (const elem of content) this._convert(elem)
    } else {
  ...
  private _convert(elem: MessageElem | string) {
    ...
    else if (Reflect.has(this, elem.type)) this[elem.type](elem as any)
  }
  ...
  private location(elem: LocationElem) {
    const { address, lat, lng, name, id } = elem
  ...
  private share(elem: ShareElem) {
    ...
    if (!url || !title) throw new Error("link share need 'url' and 'title'")
    if (title.length > 26) title = title.substr(0, 25) + '…'
    title = escapeXml(title)
    const data = `<?xml version="1.0" encoding="utf-8"?>
		<msg templateID="12345" action="web" brief="[分享] ${title}" serviceID="1" sourceName="QQ浏览器" url="${escapeXml(
      url
    )}"><item layout="2">${
  ...
  private reply(elem: ReplyElem) {
    const { id } = elem
    if (id.length > 24) this.quote({ ...parseGroupMessageId(id), message: '[消息]' })
    else this.quote({ ...parseDmMessageId(id), message: '[消息]' })
  }
