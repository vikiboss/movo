...
import type { ImageElem, FlashElem } from './elements'
...
/** 构造图片file */
export function buildImageFileParam(
  md5: string,
  size?: number,
  width?: number,
  height?: number,
...
export function parseImageFileParam(file: string) {
  let sp = file.split('-')
  const md5 = sp[0].slice(0, 32)
  const size = Number(sp[0].slice(32)) || 0
  const width = Number(sp[1]) || 0
  const height = parseInt(sp[2]) || 0
  sp = file.split('.')
  const ext = sp[1] || 'jpg'
  return { md5, size, width, height, ext }
}
...
export class Image {
  ...
  private async fromLocal(file: string) {
    try {
      ...
      const [dimensions, md5] = await Promise.all([probe(readable, true), md5Stream(readable)])
      readable.destroy()
      this.setProperties(dimensions)
      this.md5 = md5
      this.size = stat.size
