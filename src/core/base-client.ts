...
async function register(this: BaseClient, logout = false, reflush = false) {
  ...
  try {
    ...
    if (!logout) this.emit('internal.error.network', -3, 'server is busy(register)')
  }
}
  ...
  let sso = new Writer()
    .writeWithLength(
      new Writer()
        ...
        .writeU32(subappid)
        .writeBytes(
          Buffer.from([0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00])
        )
...
function decodeLoginResponse(this: BaseClient, payload: Buffer): any {
  ...
  if (type === 0) {
    ...
    return register.call(this).then(() => {
      if (this[IS_ONLINE]) this.emit('internal.online', token, nickname, gender, age)
    })
  }
