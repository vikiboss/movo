...
export class Member extends User {
  ...
  /** 禁言剩余时间 */
  get mute_left() {
    const t = (this.info?.shutup_time ?? 0) - timestamp()
    return t > 0 ? t : 0
  }
  ...
  async renew(): Promise<MemberInfo> {
    if (!this.c.gml.has(this.gid) && this.c.config.cache_group_member) this.group.getMemberMap()
  ...
  /** 踢 */
  async kick(block = false, message = '') {
    const body = pb.encode({
      1: this.gid,
      2: {
        1: 5,
